...
> **AI AGENT NOTE**: Refer to this file to understand the current implementation state of features before querying the codebase. This reduces token consumption and provides high-level context that raw code analysis might miss.
...
#### VPN Traffic Control & Monitoring

- **VPN interception & app blocking**
  - **User-facing behavior**: Intercepts IP traffic, blocks specific apps, and filters domains.
  - **Primary components**: `CDCVpnService`, `AdminVpnFragment`.
  - **Business logic**: Captures IP metadata, routes blocked apps to a black-hole, and logs DNS queries.

#### Remote App List Management

- **Fetch installed apps**
  - **User-facing behavior**: Admin views full list of apps to select for blocking.
  - **Primary components**: `FirebaseUtils.uploadUserAppList`, `AdminVpnAppListFragment`.

#### Location Tracking

- **Real-time & history tracking**
  - **User-facing behavior**: Captures live coordinates and location history.
  - **Primary components**: `FirebaseUtils.uploadLiveLocation`, `FirebaseUtils.uploadLocationHistory`.

#### Camera & Audio Capture (Experimental)

- **Snapshot & Recording**
  - **User-facing behavior**: Captures camera frames and audio recordings.
  - **Primary components**: `FirebaseUtils.uploadCameraImage`, `FirebaseUtils.uploadAudio`.

#### Remote App Update

- **OTA Updates**
  - **User-facing behavior**: Silent or prompted updates pushed by admin.
  - **Primary components**: `FirebaseUtils.listenForAppUpdate`, `scripts/firebase_update.ps1`.
...
### Experimental / Core Integrated features

- **VPN Traffic Control & Monitoring**
  - **User-facing behavior**: When enabled, the VPN intercepts all device traffic. Admin can block internet for specific apps or filter domains globally.
  - **Primary components**: `CDCVpnService`, `AdminVpnFragment`, `AdminVpnAppListFragment`.
  - **Business logic**: 
    - **Global Interception**: Captures IP traffic (v4/v6) metadata.
    - **App Blocking**: Routes restricted apps to a black-hole tunnel, disabling their internet access.
    - **Domain Filtering**: Inspects DNS queries and logs attempts to access restricted domains.
    - **Monitoring**: Logs traffic metadata (proto, src/dst IP, service, bytes) to Firebase (last 50 logs).

- **Remote App List Management**
  - **User-facing behavior**: Admin can view a full list of installed apps on the remote device and select which ones to block.
  - **Primary components**: `AdminVpnAppListFragment`, `ClickActions.CAPTURE_INSTALLED_APPS`, `FirebaseUtils.uploadUserAppList`.
  - **Business logic**:
    - **Understandable List**: Prioritizes Launcher apps with human-readable names; pushes cryptic system packages to the bottom.
    - **Remote Sync**: Triggers a device-side scan and upload of the full app list (`QUERY_ALL_PACKAGES`) to ensure accuracy.
