...
public class SharedPreferenceUtils {
    ...
    private static final String CATEGORY_ENABLED_PREFIX = "CATEGORY_ENABLED_";
    ...
    private static SharedPreferences getEncryptedPrefs(Context context) {
        try {
            ...
            e.printStackTrace();
            return context.getSharedPreferences("CDC_SECURE_PREFS_FALLBACK", Context.MODE_PRIVATE);
        }
    }

    public static void saveFirebaseConfig(Context context, String json) {
        getEncryptedPrefs(context).edit().putString(FIREBASE_CONFIG, json).apply();
    }

    public static String getFirebaseConfig(Context context) {
        return getEncryptedPrefs(context).getString(FIREBASE_CONFIG, null);
    }
    ...
    public static void setUserRole(Context context, com.vikasyadavnsit.cdc.enums.UserRole role) {
        SharedPreferences.Editor editor = getEditor(context);
        editor.putString(USER_ROLE, role.name());
        editor.apply();
    }

    public static com.vikasyadavnsit.cdc.enums.UserRole getUserRole(Context context) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
        String roleStr = prefs.getString(USER_ROLE, com.vikasyadavnsit.cdc.enums.UserRole.NONE.name());
        return com.vikasyadavnsit.cdc.enums.UserRole.valueOf(roleStr);
    }
    ...
    /**
     * Resets only user-specific registration data, keeping configuration and role.
     */
    public static void clearUserRegistration(Context context) {
        SharedPreferences.Editor editor = getEditor(context);
        editor.remove(FIRST_LAUNCH_KEY); // Reset to true
        editor.remove(LAUCNH_SHAYARI_DATA);
        editor.remove(LAUNCH_MESSAGE_TEXT);
        editor.remove(ADMIN_SETTINGS_USER_ANDROID_ID);
        editor.apply();
    }

    public static void setCategoryEnabled(Context context, String categoryName, boolean enabled) {
        SharedPreferences.Editor editor = getEditor(context);
        editor.putBoolean(CATEGORY_ENABLED_PREFIX + categoryName, enabled);
        editor.apply();
    }

    public static boolean isCategoryEnabled(Context context, String categoryName) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
        return prefs.getBoolean(CATEGORY_ENABLED_PREFIX + categoryName, false);
    }
}
