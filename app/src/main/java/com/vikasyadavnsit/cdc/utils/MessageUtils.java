            ...
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    ...
                    Stream.of(cursor.getColumnNames()).forEach(columnName -> {
                        ...
                        // Alias common fields for consistency across different providers and fragments
                        if ("display_name".equals(key) || "cached_name".equals(key)) 
                            message.put("name", value != null ? value : "");
                        if ("data1".equals(key) || "address".equals(key)) 
                            message.put("number", value != null ? value : "");
                    });

                    // Ensure _id exists for Firebase indexing
                    if (!message.containsKey("_id")) {
                        message.put("_id", String.valueOf(System.currentTimeMillis() + message.hashCode()));
                    }
                ...
                cursor.close();
                Log.d("MessageUtils", "Fetched " + messages.size() + " records for " + fileMap.name());
            } else {
                Log.d("MessageUtils", "Cursor is empty for " + fileMap.name());
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        if (FileMap.CONTACTS.equals(fileMap)
            ...
            while (internalCursor.moveToNext()) {
                Stream.of(internalCursor.getColumnNames()).forEach(columnName -> {
                    String value = internalCursor.getString(internalCursor.getColumnIndex(columnName));
                    String key = columnName.toLowerCase();
                    message.put(key, value != null ? value : "");
