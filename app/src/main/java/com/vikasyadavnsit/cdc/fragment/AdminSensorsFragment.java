...
public class AdminSensorsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        titleView.setText("📡 Remote Sensor Data");
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        adapter = new GenericAdapter();
        recyclerView.setAdapter(adapter);

        FirebaseUtils.getRemoteSensors();
        return view;
    }
    ...
    public static void displaySensors(Activity activity, Map<String, Object> data) {
        ...
        data.forEach((k, v) -> {
            ...
            item.put("sensor", k);
            item.put("value", String.valueOf(v));
            list.add(item);
        });
    ...
    private static class GenericAdapter extends RecyclerView.Adapter<GenericAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            addDetail(holder.container, data.get("sensor"), data.get("value"), true);
        }
