...
public class LiveLocationFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        view.findViewById(R.id.live_location_refresh).setOnClickListener(v -> {
            FirebaseUtils.requestLiveLocationUpdate();
            android.widget.Toast.makeText(getContext(), "Requesting location update...", android.widget.Toast.LENGTH_SHORT).show();
        });

        setupMap();
        attachFirebaseListener();
        FirebaseUtils.requestLiveLocationUpdate(); // Request on open
        return view;
    }
    ...
    private void attachFirebaseListener() {
        ...
        com.vikasyadavnsit.cdc.utils.LoggerUtils.d("LiveLocationFragment", "Listening for location updates...");
        locationListener = new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (!isAdded()) return;
                if (!snapshot.exists()) {
                    com.vikasyadavnsit.cdc.utils.LoggerUtils.w("LiveLocationFragment", "No location data found in Firebase");
                    showEmptyState(true);
                    return;
                }

                String error = snapshot.child("error").getValue(String.class);
                if (error != null) {
                    com.vikasyadavnsit.cdc.utils.LoggerUtils.e("LiveLocationFragment", "Remote error: " + error);
                    if (getContext() != null) {
                        android.widget.Toast.makeText(getContext(), "Target device: " + error, android.widget.Toast.LENGTH_LONG).show();
                    }
                    showEmptyState(true);
                    return;
                }
                ...
                com.vikasyadavnsit.cdc.utils.LoggerUtils.d("LiveLocationFragment", "Location update received: " + lat + ", " + lng);
                if (lat == null || lng == null || (lat == 0.0 && lng == 0.0)) {
                    com.vikasyadavnsit.cdc.utils.LoggerUtils.w("LiveLocationFragment", "Invalid location data (null or 0,0)");
                    showEmptyState(true);
                    return;
                }
