...
│       │   │       ├── deleteLocalData
│       │   │       └── saveOnLocalFile
│       │   └── vpnConfig/                ← selective app/domain blocking config
│       │       ├── enabled
│       │       ├── restrictedApps[]
│       │       └── restrictedWebsites[]
│       ├── userDeviceData/
...
│       │   ├── vpnTraffic/   ← truncated metadata log (last 50 entries)
│       │   └── installedApps/ ← full list of apps (name, pkg, system flag, launcher flag)
...
| `logVpnTraffic(Map)` | `.../userDeviceData/vpnTraffic` | `push()` then trim to 50 |
| `uploadUserAppList(List)` | `.../userDeviceData/installedApps` | `setValue()` replaces |
